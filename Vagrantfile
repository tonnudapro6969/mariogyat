Vagrant.configure("2") do |config|
  ...
  config.vm.define "ansible" do |ansible|
    ...
    ansible.vm.provision "shell", inline: <<-SHELL
      ...
      apt-get -y upgrade
      apt-get install -y git fish tmux mc wget
      apt-get install -y ansible tree yamllint ansible-lint
  
      # Install sshpass manually due to repository issue
      wget http://deb.debian.org/debian/pool/main/s/sshpass/sshpass_1.09-1%2bb1_amd64.deb
      dpkg -i sshpass_1.09-1+b1_amd64.deb || apt-get -f install -y
    SHELL
  end
